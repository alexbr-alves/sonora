...
import {
  ArrowDown, ArrowLeft, ArrowUp, AudioLines, Grid3X3, Library, Link2, PanelRight,
  Plus, QrCode, Search, Settings2, Trash2, Upload, Volume2, Wifi, X
} from "lucide-react";
...
type View = "deck" | "connection" | "layout" | "library" | "catalog" | "edit";
...
export default function MobileDeck(props: MobileDeckProps) {
  const { pads, layouts, currentLayoutId, playing, remote, onPlay, onScanQr, onSetCurrentLayout, onSetLayouts, onImportFiles, onAddCatalog, onDeletePad } = props;
  ...
  if (view === "catalog") {
    return <CatalogView connected={remote.connected} searchCatalog={remote.searchCatalog} playDirect={remote.playCatalog} downloadAudio={remote.downloadCatalogAudio} layouts={layouts} libraryPads={pads} onAdd={onAddCatalog} onRemove={(sourceId) => { const pad = pads.find((item) => item.sourceId === sourceId); return pad ? onDeletePad(pad.id) : Promise.resolve(); }} onBack={() => setView("library")} />;
  }
  ...
  if (view === "connection") {
    ...
    content = <Page title="Biblioteca" kicker="SEUS SONS"><div className="library-actions"><button className="glass-action primary" onClick={() => setView("catalog")}><Search size={18} /> Pesquisar online</button><button className="glass-action" onClick={() => fileInput.current?.click()}><Upload size={18} /> Anexar áudio</button></div><input ref={fileInput} hidden multiple type="file" accept="audio/*" onChange={(event) => { const files = [...(event.target.files ?? [])].filter((file) => file.type.startsWith("audio/")); setPendingFiles(files); event.target.value = ""; }} /><section className="library-list glass-list">{pads.length ? pads.map((pad) => <article key={pad.id}><button className="library-play" onClick={() => void onPlay(pad)}><span className={`sound-orb ${pad.color}`}><AudioLines size={18} /></span><strong>{pad.name}</strong></button><button aria-label={`Apagar ${pad.name}`} onClick={() => void onDeletePad(pad.id)}><Trash2 size={17} /></button></article>) : <div className="empty-library"><Library size={28} /><strong>Sua biblioteca está vazia</strong><small>Pesquise online ou anexe um áudio do aparelho.</small></div>}</section>{pendingFiles.length > 0 && <LayoutPicker layouts={layouts} title={`Adicionar ${pendingFiles.length} ${pendingFiles.length === 1 ? "som" : "sons"} em:`} onChoose={(id) => void chooseFiles(id)} onClose={() => setPendingFiles([])} />}</Page>;
  } else {
    content = <main className="stream-deck" onPointerDown={(event) => { deckPointerStart.current = { x: event.clientX, y: event.clientY }; }} onPointerUp={(event) => finishDeckSwipe(event.clientX, event.clientY)} onPointerCancel={() => { deckPointerStart.current = null; }}><div className="deck-badge"><span className={`deck-status ${remote.connected ? "online" : ""}`} /><strong>{currentLayout.name}</strong></div><section className="stream-grid" style={{ gridTemplateColumns: `repeat(${currentLayout.columns},minmax(0,1fr))`, gridTemplateRows: `repeat(${currentLayout.rows},minmax(0,1fr))` }}>{slots.map((pad, index) => pad ? <button key={pad.id} className={`deck-key ${pad.color} ${playing === pad.id ? "playing" : ""}`} onClick={() => { if (!blockDeckTap.current) void onPlay(pad); }}><span className="key-sheen" /><div>{playing === pad.id ? <Volume2 size={30} /> : <AudioLines size={30} />}</div><strong>{pad.name}</strong></button> : <div key={`empty-${index}`} className="deck-key empty" aria-hidden="true" />)}</section>{layouts.length > 1 && <nav className="layout-dots" aria-label="Layouts">{layouts.map((layout) => <button key={layout.id} className={layout.id === currentLayout.id ? "active" : ""} aria-label={`Abrir layout ${layout.name}`} aria-current={layout.id === currentLayout.id ? "page" : undefined} onClick={() => onSetCurrentLayout(layout.id)} />)}</nav>}</main>;
  }

  return <>{content}<button className={`menu-trigger ${menuOpen ? "open" : ""}`} aria-label={menuOpen ? "Fechar menu" : "Abrir menu"} onClick={() => setMenuOpen((open) => !open)}><PanelRight size={19} /></button>{menuOpen && <><button className="menu-scrim" aria-label="Fechar menu" onClick={() => setMenuOpen(false)} /><aside className="deck-menu"><div className="menu-brand"><span className="brand-glow"><AudioLines size={19} /></span><div><strong>Sonora</strong><small>{remote.connected ? "Conectado" : "Modo local"}</small></div></div><nav><MenuItem icon={<Grid3X3 />} label="Deck" active={view === "deck"} onClick={() => navigate("deck")} /><MenuItem icon={<Link2 />} label="Conexão" active={view === "connection"} onClick={() => navigate("connection")} /><MenuItem icon={<Settings2 />} label="Layout" active={view === "layout" || view === "edit"} onClick={() => navigate("layout")} /><MenuItem icon={<Library />} label="Biblioteca" active={view === "library"} onClick={() => navigate("library")} /></nav></aside></>}</>;
}
